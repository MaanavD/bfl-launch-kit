...
from flux_api import restyle_image, MODELS

log = logging.getLogger("fluxbot.restyle")


class Restyle(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.storage = bot.storage

    @app_commands.command(name="restyle", description="Transform an image into a new style")
    @app_commands.describe(
        prompt="Describe how to transform the image",
        image="Upload the image you want to restyle",
    ...
    async def restyle_cmd(
        ...
        if not image.content_type or not image.content_type.startswith("image/"):
            await interaction.response.send_message(
                "Please upload an image file (PNG, JPG, etc.)", ephemeral=True
            )
            return
        ...
        try:
            image_bytes = await restyle_image(
                api_key, full_prompt, image.url, model=model
            )
        except (ValueError, TimeoutError) as e:
            await interaction.followup.send(f"**Restyle failed:** {e}")
            return

        file = discord.File(BytesIO(image_bytes), filename="fluxbot_restyle.jpg")
        embed = discord.Embed(color=0x7C3AED)
        embed.set_author(name=f"{interaction.user.display_name}'s restyle", icon_url=interaction.user.display_avatar.url)
        embed.description = f"**Prompt:** {prompt}"
        if user_style:
            embed.set_footer(text=f"Style: {user_style}")
        embed.set_image(url="attachment://fluxbot_restyle.jpg")
        await interaction.followup.send(embed=embed, file=file)


async def setup(bot: commands.Bot):
    await bot.add_cog(Restyle(bot))
